 ...
 */
package deconvolutionSACDlab.module.dropdownbuttons.gpl.swingcomponents;
...
public class JSplitButton extends JButton implements MouseMotionListener, MouseListener, ActionListener, Serializable {
	...
	private boolean						hidePopupOnText				= true;
	 ...
	 */
	public JSplitButton(String text, boolean hidePopupOnText) {
		super(text);
		this.hidePopupOnText = hidePopupOnText;
	...
	/**
	 * Returns the separatorSpacing. Separator spacing is the space above and below
	 * the separator( the line drawn when you hover your mouse over the split part
	 * of the button).
	 * 
	...
	/**
	 * Sets the separatorSpacing.Separator spacing is the space above and below the
	 * separator( the line drawn when you hover your mouse over the split part of
	 * the button).
	 * 
	 ...
	 *            true to show the attached dropdown even if the button part is
	...
	/**
	 * Gets the image to be drawn in the split part. If no is set, a new image is
	 * created with the triangle.
	 * 
	 * @return image
	 */
	 ...
	 * listener is the currently set <code>Action</code> for the button, then the
	...
	public void actionPerformed(ActionEvent e) {
		if (popupMenu == null) {
			...
			if (hidePopupOnText) popupMenu.show(this, getWidth() - (int) popupMenu.getPreferredSize().getWidth(), getHeight());
			fireButtonClicked(e);
		}
	}
