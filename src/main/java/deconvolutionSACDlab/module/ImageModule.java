...
public class ImageModule extends AbstractImageSelectionModule implements ActionListener {
	...
	public ImageModule() {
		super("image");
		choose = new ChooseImageDropDownButton("image", this, "Choose");
		show = new ShowImageDropDownButton("image","Check");
		create("Image", "-image", show, choose);
	}
	...
	public String getCommand() {
		...
		if (row < 0)
			return "";
		String cmd = "-image " + table.getCell(row, 1) + " " + table.getCell(row, 2);
		if (show != null)
	...
	public void update() {
		...
		if (row >= 0) {
			...
			setSynopsis("");
			setCommand("<span style='color:#F03030'>Drag your input image, here</span>");
		}
